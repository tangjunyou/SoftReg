...
# Shared estimate used only to pick enough material; final pagination is fixed
# by build_code_pdf.py (52 rows/page), not by Word.
...
def final_confirmation_issue(workdir: Path) -> str | None:
    """Return an issue string when drafts changed after the final markdown confirmation."""
    path = workdir / "草稿/最终生成确认.json"
    if not path.is_file():
        return None
    try:
        data = read_json(path)
    except Exception:
        return "草稿/最终生成确认.json 无法读取，请重新确认"
    if not data.get("markdown_confirmed"):
        return None
    stored = data.get("draft_file_sha256")
    if stored and stored != draft_snapshot(workdir):
        return "草稿在最终确认后已被修改，原确认已失效，请重新运行 confirm_stage.py --stage markdown"
    return None
    ...
    if review_path.is_file():
        try:
            ...
            if not isinstance(rounds, list) or not rounds:
                issues.append("操作手册自检记录缺少轮次信息")
            else:
                last_issues = rounds[-1].get("issues") if isinstance(rounds[-1], dict) else None
                if last_issues:
